...
import { getReminders, getPendingForms, getAutoReminders, getCustomReminder, saveSemesterDates, resetAutoReminders, resetCustomReminders, saveAutoReminders, saveCustomReminder, sendQuickReminder, } from "../../api/reminders";
...
export default function Reminders() {
  ...
  const handleSaveSemester = async () => {
    if (!semesterStart || !semesterEnd) {
      handleShowSnackbar("Please select both start and end dates", "error");
      return;
    }

    const start = semesterStart.toISOString().split("T")[0];
    const end = semesterEnd.toISOString().split("T")[0];

    try {
      // 1. Save semester dates
      await saveSemesterDates(settings, start, end);

      // 2. Reset auto reminders to default (all false)
      await resetAutoReminders();

      // 3. Reset custom reminders to default (empty dates)
      await resetCustomReminders();
      ...
      // 5. Trigger component reset
      setResetKey(prev => prev + 1);

      handleShowSnackbar("Semester dates saved! Auto and custom reminders have been reset.", "success");
    } catch (error) {
      handleShowSnackbar("Error saving semester dates", "error");
    }
  };
