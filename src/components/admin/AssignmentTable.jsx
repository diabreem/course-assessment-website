...
import { getCampusesByUniversityId } from "../../api/university";
    ...
    const getEmptyAssignment = () => {
        ...
        if (campuses && campuses.length > 0) {
            campuses.forEach(campus => {
                base[`semester_${campus.name}`] = "";
                base[`instructor_id_${campus.name}`] = "";
            });
        }
        return base;
    };
 ...
 React.useEffect(() => {
    ...
    const getGroupedAssignments = (yearNumber, academicYear) => {
        ...
        const grouped = soList.map(so => {
            ...
            yearAssignments
                ...
                .forEach(assignment => {
                    ...
                    if (!courseAssignments[key]) {
                        ...
                        campuses.forEach(campus => {
                            courseAssignments[key][campus.name] = null;
                        });
                    }
            ...
            const courseRows = Object.values(courseAssignments).map(data => {
                ...
                campuses.forEach(campus => {
                    campusAssignments[campus.name] = data[campus.name];
                });
                return {
                    course,
    ...
    const getPCsBySO = (soId) => {
        ...
        const filteredPCs = PCs.filter(pc => {
            const pcSoId = typeof pc.so_id === 'string' ? parseInt(pc.so_id) : pc.so_id;
            return pcSoId === soIdNum;
        });
        return filteredPCs;
    };
    ...
    const filterInstructors = (campusName) => {
        const filtered = users.filter((user) => {
            ...
            const isCampusMatch = user.campus === campusName || 
                                 user.campus === "NULL" || 
                                 !user.campus || 
                                 user.campus === "";
    ...
    const handlePCChange = (e) => {
        ...
        setAssignmentData(prev => {
            const newData = {
                ...prev,
                pc_ids: Array.isArray(selectedPCIds) ? selectedPCIds : [selectedPCIds]
            };
            return newData;
        });
    };
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            campuses.forEach(campus => {
                const semesterField = `semester_${campus.name}`;
                const instructorField = `instructor_id_${campus.name}`;
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            campuses.forEach(campus => {
                const semesterField = `semester_${campus.name}`;
                const instructorField = `instructor_id_${campus.name}`;
                const campusAssignment = existingAssignments.find(a => a.campus === campus.name);
                if (assignmentData[semesterField] && assignmentData[instructorField]) {
                    const campusData = {
                        course_id: typeof assignmentData.course_id === 'string' ? parseInt(assignmentData.course_id) : assignmentData.course_id,
                        so_id: typeof assignmentData.so_id === 'string' ? parseInt(assignmentData.so_id) : assignmentData.so_id,
    ...
    const validateForm = () => {
        ...
        const hasAtLeastOneCampus = campuses.some(campus => {
            const hasSemester = !!assignmentData[`semester_${campus.name}`];
            const hasInstructor = !!assignmentData[`instructor_id_${campus.name}`];
    ...
    const openAddDialog = (so, yearNumber) => {
        ...
        campuses.forEach(campus => {
            newAssignment[`semester_${campus.name}`] = currentSemester;
        });
    ...
    const openEditDialog = (assignmentsByCampus, yearNumber) => {
        ...
        const allPCIds = [];
        campuses.forEach(campus => {
            if (assignmentsByCampus[campus.name]?.pc_ids) {
                const pcIds = assignmentsByCampus[campus.name].pc_ids;
        ...
        campuses.forEach(campus => {
            const assignment = assignmentsByCampus[campus.name];
            newAssignmentData[`semester_${campus.name}`] = assignment?.semester || "";
            newAssignmentData[`instructor_id_${campus.name}`] = assignment?.instructor_id?.toString() || "";
        });
                    ...
                    <Box className="flex flex-col gap-4 pt-4">
                        ...
                        {campuses.map(campus => (
                            <Box key={campus.id} className="border p-4 rounded-lg">
                                <Typography variant="h6" className="mb-3 pb-5">{campus.name} Campus</Typography>
                                <Box className="grid grid-cols-3 gap-4">
                                    <FormControl fullWidth>
                                        <InputLabel id={`semester-${campus.name}-label`}>Semester ({campus.name})</InputLabel>
                                        <Select
                                            labelId={`semester-${campus.name}-label`}
                                            value={assignmentData[`semester_${campus.name}`] || ""}
                                            onChange={(e) => handleFieldChange(`semester_${campus.name}`, e.target.value)}
                                            label={`Semester (${campus.name})`}
                                        >
                                    ...
                                    <FormControl fullWidth>
                                        <InputLabel id={`instructor-${campus.name}-label`}>Instructor ({campus.name})</InputLabel>
                                        <Select
                                            labelId={`instructor-${campus.name}-label`}
                                            value={assignmentData[`instructor_id_${campus.name}`] || ""}
                                            onChange={(e) => handleFieldChange(`instructor_id_${campus.name}`, e.target.value)}
                                            label={`Instructor (${campus.name})`}
                                        >
                                            <MenuItem value="">Select Instructor</MenuItem>
                                            {filterInstructors(campus.name).map(user => (
                                                        ...
                                                        {group.courseRows.map((row, rowIndex) => {
                                                            return (
                                                                ...
                                                                >
                                                                    ...
                                                                    {campuses.map(campus => {
                                                                        const assignment = row.campusAssignments?.[campus.name];
                                                                        return (
                                                                            <React.Fragment key={campus.id}>
                                                                                <TableCell sx={{ 
                                                                                    borderRight: '1px solid #e0e0e0',
                                                                            ...
                                                                            <Box className="flex gap-0.5">
                                                                                <IconButton
                                                                                    ...
                                                                                    onClick={() => {
                                                                                        ...
                                                                                        campuses.forEach(campus => {
                                                                                            assignmentsByCampus[campus.name] = row.campusAssignments?.[campus.name];
                                                                                        });
